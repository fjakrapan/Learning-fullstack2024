...
app.post('/hello', (req, res) => {
  //ชื่อ hello ซ้ำได้ ถ้าคนละ method
  res.send(req.body);
});

app.put('/myput', (req, res) => {
  // ใช้ในการอัพเดทข้อมูลในdatabase
  res.send(req.body);
});
...
app.get('/book/sum', async (req, res) => {
  try {
    const data = await prisma.book.aggregate({
      //aggregate ใช้เมื่อต้องหาค่าหรือคำนวณ
      _sum: {
        price: true,
      },
    });
...
app.get('/user/verifyToken', (req, res) => {
  try {
    const secret = process.env.TOKEN_SECRET;
    const token =
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTAwLCJuYW1lIjoiZnJhbmsiLCJsZXZlbCI6ImFkbWluIiwiaWF0IjoxNzE5OTc4MTc5LCJleHAiOjE3MjAwNjQ1Nzl9.P6M-w06xIc_qQTai7yxMODxNjFxw9mUoeEAbliQD-yg';
...
app.get('/fileExists', (req, res) => {
  //เช็คว่ามีไฟลืนี้อยู่จริงหรือเปล่า
  try {
    const fs = require('fs');
...
app.get('/createPdf', (req, res) => {
  try {
    ...
    doc.addPage().fontSize(25).text('สวัสดี ทดสอบสร้างภาพ pdf ....', 100, 250);
    doc.end();
